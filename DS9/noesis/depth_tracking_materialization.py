...
def _native_ext_dir() -> Path:
    return Path(os.environ.get("NOESIS_NATIVE_EXT_DIR", REPO_ROOT)).expanduser().resolve()
...
def ensure_native_object_depth_extension(logger: logging.Logger | None = None) -> None:
    ...
    if ext_path.exists() and ext_path.stat().st_size > 0 and ext_path.stat().st_mtime >= source_path.stat().st_mtime:
        return
    _run([str(_native_build_script("build_noesis_depth_meta_ext.sh"))], logger=logger)
...
def ensure_native_depth_tracking_tensor_extension(logger: logging.Logger | None = None) -> None:
    ...
    if ext_path.exists() and ext_path.stat().st_size > 0 and ext_path.stat().st_mtime >= source_path.stat().st_mtime:
        return
    _run([str(_native_build_script("build_noesis_depth_tracking_tensor_ext.sh"))], logger=logger)
