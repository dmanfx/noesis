...
from .native_artifact_provenance import (
...
ONNX_DIR = Path(os.environ.get("NOESIS_ONNX_DIR", REPO_ROOT / "models" / "onnx")).expanduser().resolve()
ENGINE_DIR = Path(os.environ.get("NOESIS_ENGINE_DIR", REPO_ROOT / "models" / "engines")).expanduser().resolve()
PIPELINE_DIR = Path(os.environ.get("NOESIS_PIPELINE_DIR", REPO_ROOT / "pipelines")).expanduser().resolve()
...
_CONFIG_TEMPLATE = PIPELINE_DIR / "config_infer_secondary_depth_tracking_da2.template.ini"


def _native_ext_dir() -> Path:
    return Path(
        os.environ.get("NOESIS_NATIVE_EXT_DIR", REPO_ROOT / "native_extensions")
    ).expanduser().resolve()


def _native_build_script(name: str) -> Path:
    script_dir = Path(os.environ.get("NOESIS_NATIVE_BUILD_SCRIPT_DIR", REPO_ROOT / "scripts")).expanduser().resolve()
...
def _validated_asset_paths(
    *,
    ...
    if (width, height) != DEFAULT_INPUT_SIZE:
        raise ValueError(
            "Only the approved DS9 depth-tracking operating point is currently supported: "
...
def ensure_native_object_depth_extension(logger: logging.Logger | None = None) -> None:
    ...
    ext_path = _native_ext_dir() / f"noesis_depth_meta_ext{ext_suffix}"
    source_path = REPO_ROOT / "native" / "noesis_depth_meta_ext.cpp"
    build_script = _native_build_script("build_noesis_depth_meta_ext.sh")
    ...
    try:
        attest_ds9_native_artifacts(
            ds9_root=REPO_ROOT,
            native_dir=_native_ext_dir(),
            module_names=("noesis_depth_meta_ext",),
        )
    except DS9NativeArtifactProvenanceError as exc:
        raise RuntimeError(
            f"Object depth extension provenance is invalid: {exc}. "
            f"Run {build_script} before runtime startup."
        ) from exc
...
def ensure_native_depth_tracking_tensor_extension(logger: logging.Logger | None = None) -> None:
    ...
    ext_suffix = str(sysconfig.get_config_var("EXT_SUFFIX") or ".so")
    ext_path = _native_ext_dir() / f"noesis_depth_tracking_tensor_ext{ext_suffix}"
    source_path = REPO_ROOT / "native" / "noesis_depth_tracking_tensor_ext.cpp"
    build_script = _native_build_script("build_noesis_depth_tracking_tensor_ext.sh")
    ...
    try:
        attest_ds9_native_artifacts(
            ds9_root=REPO_ROOT,
            native_dir=_native_ext_dir(),
            module_names=("noesis_depth_tracking_tensor_ext",),
        )
    except DS9NativeArtifactProvenanceError as exc:
        raise RuntimeError(
            f"Depth tracking tensor extension provenance is invalid: {exc}. "
            f"Run {build_script} before runtime startup."
        ) from exc
