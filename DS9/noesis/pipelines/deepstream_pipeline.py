...
except Exception:  # pragma: no cover - optional dependency or older Service Maker build
...
class DepthGateOperator(_BufferOperatorBase):  # pragma: no cover - runtime only
    ...
    def __init__(self, pipeline: DeepStreamPipeline) -> None:
...
_PIPELINE_SINGLETON: Optional[DeepStreamPipeline] = None
...
def _attach_depth_gate(pipeline: DeepStreamPipeline) -> None:
...
def _attach_fps_probes(pipeline: DeepStreamPipeline) -> None:
    """Optionally attach lightweight FPS probes to key nodes for debugging.

    Enabled only when NOESIS_DS9_FPS_PROBE is truthy (\"1\", \"true\", \"yes\", \"on\").
    ...
    flag = os.environ.get("NOESIS_DS9_FPS_PROBE", "")
    ...
    for node_name in debug_nodes:
        ...
        try:
            ...
            logger.info("FPS probe attached to '%s' (NOESIS_DS9_FPS_PROBE=1)", node_name)
        except Exception as exc:  # pragma: no cover - runtime dependent
            pipeline.errors.append(f"fps_probe_attach:{node_name}:{exc}")


def _attach_latency_probe(pipeline: DeepStreamPipeline) -> None:
    ...
    if pipeline.latency_collector is None:
        try:
            window_sec = float(os.environ.get("NOESIS_DS9_LATENCY_WINDOW_SEC", "10") or "10")
        except Exception:
            window_sec = 10.0
        pipeline.latency_collector = LatencyCollector(window_sec=window_sec)

    flag = os.environ.get("NOESIS_DS9_LATENCY_PROBE")
...
def _attach_source_progress_probes(pipeline: DeepStreamPipeline) -> None:
...
def build_pipeline(yaml_path: str | Path) -> DeepStreamPipeline:
    """Create the canonical pipeline skeleton using Service Maker primitives."""
    ...
    if use_stub:
        ...
        if DSPipeline is not None:
            try:
                ds_pipeline = DSPipeline("noesis-ds9")
            except Exception as exc:  # pragma: no cover - depends on DS backend
    ...
    if per_source_pipeline:
        ...
        loop_local_mp4 = _env_truthy("NOESIS_DS9_LOOP_LOCAL_MP4", default=True)
        ...
        multi_cfg: Dict[str, Any] = {
            ...
            # - Disable REST control API (port=0) since Noesis drives URIs from YAML.
            "drop-pipeline-eos": 1,
        ...
        if loop_local_mp4 and local_mp4_indices:
            ...
            logger.info("DS9: enabled nvmultiurisrcbin file-loop for local mp4 sources: %s", local_mp4_indices)
    ...
    tracker_cfg_raw = cfg.get(
        "tracker", {"config-file": "DS9/config/nvtracker.yaml"}
    )
    ...
    # Insert a tiled renderer stage in the canonical path. This avoids per-camera branches.
    ...
    logger.info(
        json.dumps(
            {
                "event": "ds9_zero_copy_memory_config",
...
def get_pipeline() -> DeepStreamPipeline:
...
def prepare(on_message=None) -> bool:
    ...
    if pipeline.errors:
        logger.error("Cannot prepare DS9 pipeline; errors present: %s", pipeline.errors)
        return False
    if pipeline.ds_pipeline is None:
        pipeline.errors.append("pyservicemaker:unavailable")
        logger.error("Cannot prepare DS9 pipeline; pyservicemaker unavailable")
        return False
    try:
        ...
        if isinstance(result, int) and result != 1:
            ...
            logger.error("DS9 pipeline prepare returned non-success code: %s", result)
        ...
        logger.error("DS9 pipeline prepare failed: %s", exc)
        pipeline.prepared = False
        return False
...
def activate() -> bool:
    ...
    if not pipeline.prepared:
        if not prepare():
            return False
    # Attempt to start the underlying Service Maker pipeline if available.
    ds = getattr(pipeline, "ds_pipeline", None)
    if ds is None:
        logger.error("DS9 bindings not available; activation aborted")
    ...
    try:
        ...
        logger.error("DS9 pipeline activation failed: %s", exc)
        pipeline.activated = False
        return False
