...
def _onnx_dir() -> Path:
    return Path(os.environ.get("NOESIS_ONNX_DIR", _model_dir())).expanduser().resolve()


def _engine_dir() -> Path:
    return Path(os.environ.get("NOESIS_ENGINE_DIR", REPO_ROOT / "models" / "engines")).expanduser().resolve()
...
def resolve_yolo26_assets(size: str) -> Dict[str, Path]:
    ...
    return {
        "template": (pipeline_dir / "config_infer_primary_yolo26_seg.template.ini").resolve(),
        ...
        "labels": (model_dir / "coco_labels.txt").resolve(),
        "parser": (pipeline_dir / "nvdsinfer_yolo26_seg" / "libnvdsinfer_yolo26_seg.so").resolve(),
