...
## DS8 Pipeline Integration

- Configured via `config/infer.yaml` `analytics.exclude` block (default element `nvdsroiexclude`, config-file `config/config_nvdsanalytics_exclude.ini`).
- Built in `noesis/pipelines/ds8_pipeline.py` as `analytics_exclude` and linked before `nvtracker`.
- Runtime reload: `noesis/server/analytics_api.py` regenerates the exclusion INI (see `_persist_exclude_ini`) and triggers reload via `attach_analytics_reload_bridge`.
- Env overrides:
...
Source: `csrc/nvdsroiexclude/`
- Build
  - `cmake -S csrc/nvdsroiexclude -B build/nvdsroiexclude -DCMAKE_BUILD_TYPE=Release`
  - `cmake --build build/nvdsroiexclude --parallel`
- Install
  - System (requires sudo):
    - `sudo install -m0755 build/nvdsroiexclude/libgstnvdsroiexclude.so \
       /opt/nvidia/deepstream/deepstream/lib/gst-plugins/libgstnvdsroiexclude.so`
  - Local (dev):
    - `export GST_PLUGIN_PATH=$PWD/csrc/nvdsroiexclude:/opt/nvidia/deepstream/deepstream/lib/gst-plugins`
- Verify
...
- Plugin not found: verify `GST_PLUGIN_PATH` or installation under DeepStream’s `lib/gst-plugins`, and `LD_LIBRARY_PATH=/opt/nvidia/deepstream/deepstream/lib`.
