...
- [`deepstream_pipeline.py`](../../../../DS9/noesis/pipelines/deepstream_pipeline.py)
- [`bev.py`](../../../../noesis/telemetry/bev.py)
- [`config_infer_secondary_mapanything.ini`](../../../../DS9/pipelines/config_infer_secondary_mapanything.ini)
...
- [`ds9_runtime_core.py`](../../../../DS9/noesis/ds9_runtime_core.py)
- [`infer.yaml`](../../../../DS9/config/infer.yaml)
...
- ReID, pose, depth, and V3DT data extraction are edge metadata/identity inputs,
  ...
  CPU; see [`hooks.py`](../../../../DS9/noesis/pipelines/hooks.py).
...
| --- | --- | --- | --- | --- |
| Optional import | `import pyds` with `PYDS_AVAILABLE` | [`hooks.py`](../../../../DS9/noesis/pipelines/hooks.py) | Module-wide optional compatibility | Keep only if a documented DS9 gap remains; otherwise quarantine behind a compatibility module. |
| Raw frame tensor iterator | `_iter_frame_tensor_meta` walks `frame_user_meta_list`, casts `NvDsUserMeta`, checks `NVDSINFER_TENSOR_OUTPUT_META`, casts `NvDsInferTensorMeta` | [`hooks.py`](../../../../DS9/noesis/pipelines/hooks.py) | Not used by the exact MapAnything buffer-operator path | Keep quarantined; it is not selected by MapAnything. |
| Raw tensor dtype mapping | `_layer_dtype` imports `NvDsInferDataType` | [`hooks.py`](../../../../DS9/noesis/pipelines/hooks.py) | Used by quarantined pyds compatibility code | Keep out of the canonical MapAnything path. |
| Raw tensor buffer copy | `_numpy_from_layer` can call `pyds.get_ptr(layer.buffer)` and copy layer bytes into NumPy | [`hooks.py`](../../../../DS9/noesis/pipelines/hooks.py) | Used by quarantined pyds compatibility code | Keep out of production MapAnything selection. |
| Raw layer extraction | `_extract_tensor_layers` can use `pyds.get_nvds_LayerInfo` | [`hooks.py`](../../../../DS9/noesis/pipelines/hooks.py) | Used by quarantined pyds compatibility code | Keep isolated from MapAnything. |
...
| Generic pyds helpers | `_resolve_pyds_cast`, `_resolve_pyds_attr`, `_iter_meta_entries` | [`hooks.py`](../../../../DS9/noesis/pipelines/hooks.py) | Support quarantined compatibility traversal | Keep behind the explicit compatibility gate where still required. |
| Analytics raw user meta | `_extract_analytics_obj_meta` walks object user meta and casts `NvDsAnalyticsObjInfo` | [`hooks.py`](../../../../DS9/noesis/pipelines/hooks.py) | Compatibility path; the production Service Maker analytics path uses `nvdsanalytics_obj_items` | Keep quarantined from production selection. |
| Analytics raw object builder | `_build_track_dict` uses non-Service-Maker object fields and pyds analytics extraction | [`hooks.py`](../../../../DS9/noesis/pipelines/hooks.py) | Production path uses `_build_track_dict_ds8` | Keep quarantined from production selection. |
| OSD label raw fallback | `_OsdLabelProcessor.handle_frame` uses pyds-style object iteration | [`hooks.py`](../../../../DS9/noesis/pipelines/hooks.py) | Production operator calls `handle_frame_ds8` | Keep quarantined; retain the Service Maker object-items path. |
