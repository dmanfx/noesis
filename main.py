...
from geometry.transform import (
    pixel_to_world as pixel_to_world_aligned,
    build_align_matrix,
)
...
class ApplicationManager:
    ...
    def __init__(self, config: AppConfig):
        ...
        self._intrinsics_callback_registered = False
        ...
        self.perf_monitor = (
            PerformanceMonitor() if config.processing.ENABLE_PROFILING else None
        )
        ...
        if config.processing.ENABLE_PROFILING:
            ...
            self.logger.info(
                f"✅ Comprehensive CPU profiling started, data: {profile_data_file}"
            )
    ...
    def _graceful_exit(self):
        ...
        try:
            ...
            if self.running:
                self.logger.error(
                    f"Graceful shutdown timed out after {MAX_SHUTDOWN_TIME}s, forcing exit"
                )
                os._exit(1)
            else:
    ...
    def initialize(self):
        ...
        try:
            ...
            print(
                f"✅ Config properties accessed: host={host}, port={port}, trail_state={trail_state}",
                flush=True,
            )
            sys.stdout.flush()
            ...
            self.websocket_server = WebSocketServer(
                ...
                depth_panel_callback=self._handle_depth_panel_state,
                initial_trail_state=trail_state,
            )

            # Add detection configuration callbacks
            self.websocket_server.detection_config_callback = (
                self._handle_detection_config_update
            )
            self.websocket_server.detection_toggle_callback = (
                self._handle_detection_toggle
            )
            ...
            try:
                ...
                if self.websocket_server:
                    self.websocket_server.calibration_getter = (
                        self._get_calibration_bundle
                    )
                    self.websocket_server.pixel_to_world_handler = (
                        self._pixel_to_world_rpc
                    )
                    self.websocket_server.set_extrinsics_handler = (
                        self._set_extrinsics_rpc
                    )
                    self.websocket_server.set_align_handler = self._set_align_rpc
                    if self.depth_source and self.websocket_server:
                        self.websocket_server.ma_depth_provider = (
                            self.depth_source.load_latest_depth
                        )
                        self.websocket_server.floorplan_provider = (
                            ...
                            max_extent=20.0,
                            cache_only=False,
                            **_: self.depth_source.generate_topdown_floorplan(
                                str(cam) if cam else "",
                                max_age_sec=max_age,
                                grid_res_m=grid_res,
        ...
        if self.config.cameras.VIDEO_FILES:
            for i, video_file in enumerate(self.config.cameras.VIDEO_FILES):
                if os.path.exists(video_file):
                    ...
                    self.logger.info(
                        f"Added video file as source: {video_file} with ID {camera_id}"
                    )
                else:
        ...
        if self.config.cameras.RTSP_STREAMS:
            for i, stream_config in enumerate(self.config.cameras.RTSP_STREAMS):
                ...
                if not stream_config.get("enabled", True):
                    self.logger.info(
                        f"Skipping disabled stream: {stream_config.get('name', f'Camera {i + 1}')}"
                    )
                    continue
                ...
                self.logger.info(
                    f"Added RTSP stream as source: {stream_name} ({stream_url}) - {stream_width}x{stream_height}"
                )
    ...
    def start(self):
        ...
        try:
            ...
            if self.multi_stream_processor and self.depth_source:
                try:
                    if not self._intrinsics_callback_registered:
                        self.depth_source.set_intrinsics_ready_callback(
                            self._handle_intrinsics_ready
                        )
                        self._intrinsics_callback_registered = True
                    sensor_mapping = {}
                    for sensor_id, info in (
                        getattr(self.multi_stream_processor, "source_info", {}) or {}
                    ).items():
                        try:
                            sensor_int = int(sensor_id)
                        except Exception:
                            continue
                        clean = info.get("clean_name") or info.get("name")
                        if clean:
                            sensor_mapping[str(clean)] = sensor_int
                        sensor_mapping[str(sensor_int)] = sensor_int
                    if sensor_mapping:
                        self.depth_source.configure_sensor_mapping(sensor_mapping)
                except Exception as e:
                    self.logger.warning(
                        f"Unable to configure MapAnything sensor mapping: {e}"
                    )
            ...
            try:
                if self.websocket_server and self.multi_stream_processor:
                    ...
                    self.logger.info(
                        "✅ Provided WebSocket server to multi-stream processor (post-start)"
                    )
                    # Avoid starting duplicate JPEG processing loop
                    if (
                        not hasattr(self, "jpeg_thread")
                        or not getattr(self, "jpeg_thread").is_alive()
                    ):
                        self._start_jpeg_processing_loop()
                        self._start_mapanything_scheduler()
            except Exception as e:
                self.logger.warning(
                    f"Unable to attach WebSocket server to processor: {e}"
                )
            ...
            print(
                f"📡 Connect to: ws://{self.config.websocket.HOST}:{self.config.websocket.PORT}"
            )
    ...
    def _start_multi_stream_processor(self):
        ...
        enabled_sources = [
            source_config
            for source_config in self.camera_sources.values()
        ...
        self.logger.info(
            f"🎥 Creating single multi-stream DeepStream processor for {len(enabled_sources)} sources"
        )

        try:
            # Create single multi-stream processor
            processor = create_deepstream_video_processor(
                sources=enabled_sources, config=self.config
            )
            ...
            try:
                self.multi_stream_processor.depth_source = self.depth_source
                self.multi_stream_processor.calibration_bundle = self.calibration_bundle
            except Exception as exc:
                self.logger.debug(f"Unable to wire depth sources into processor: {exc}")
            self.logger.info(
                f"✅ Multi-stream DeepStream processor started successfully with {len(enabled_sources)} streams"
            )
            try:
            ...
            try:
                disable_occ = os.environ.get("NOESIS_DISABLE_OCCUPANCY") == "0"
                if (
                    getattr(self.config.integrations, "ENABLE_OCCUPANCY_PUBLISH", False)
                    and not disable_occ
                ):
                    self.logger.info(
                        "🔌 Initializing OccupancyPublisher (MQTT + Influx)"
                    )
                    try:
                        print(
                            f"🔌 OccupancyPublisher enabled: MQTT {self.config.integrations.MQTT_HOST}:{self.config.integrations.MQTT_PORT} base={self.config.integrations.BASE_TOPIC} | Influx bucket={self.config.integrations.INFLUX_BUCKET_RAW}"
                        )
                    except Exception:
                        pass
                    ...
                    occ_cfg = OccupancyConfig(
                        ...
                        influx_bucket_raw=str(
                            self.config.integrations.INFLUX_BUCKET_RAW
                        ),
                    )
                    pub = OccupancyPublisher(
                        occ_cfg, logger=logging.getLogger("OccupancyPublisher")
                    )
                    ...
                    try:
                        ...
                        self.logger.info(
                            "✅ Attached OccupancyPublisher to DeepStream processor"
                        )
                        try:
                            print(
                                "✅ OccupancyPublisher attached to DeepStream processor"
                            )
                        except Exception:
                            pass
                    except Exception as e:
                        self.logger.warning(
                            f"Unable to attach OccupancyPublisher to processor: {e}"
                        )
                else:
                    self.logger.info(
                        "Occupancy publishing disabled via config or environment"
                    )
            except Exception as e:
            ...
            try:
                ...
                if self.websocket_server and (self.calibration_bundle is not None):
                    self.websocket_server.broadcast_sync(
                        {"type": "calibration-bundle", "data": self.calibration_bundle}
                    )
                    self.logger.info(
                        "📡 Re-broadcast calibration-bundle with canonical camera IDs"
                    )
            except Exception as e:
                self.logger.warning(
                    f"Calibration rebuild after DeepStream init failed: {e}"
                )
    ...
    def _start_mapanything_service(self) -> None:
        ...
        script_path = (
            project_root / "services" / "mapanything_svc" / "run.sh"
        ).resolve()
        if not script_path.exists():
            self.logger.warning(
                "MapAnything run script missing at %s; skipping service startup",
                script_path,
            )
            return
        ...
        if self._mapanything_process and self._mapanything_process.poll() is None:
            self.logger.info(
                "MapAnything service already running (pid=%s)",
                self._mapanything_process.pid,
            )
            return
        ...
        port_str = _parse(service_cfg.get("port"), "8003")
        try:
            port = int(port_str)
        except ValueError:
            port = 8003
        ...
        try:
            ...
            self.logger.error(
                "MapAnything service failed to report healthy; terminating process"
            )
            self._terminate_mapanything_process()
            raise

    def _wait_for_mapanything_ready(
            ...
            try:
                ...
                try:
                    ...
                    if response.status == 200:
                        self.logger.info(
                            "MapAnything service healthy at %s:%s", host, port
                        )
                        return
                finally:
                    conn.close()
    ...
    def _terminate_mapanything_process(self) -> None:
        """Terminate MapAnything service process group if running."""
        proc = self._mapanything_process
        if not proc:
            return
        print(f"DEBUG: Terminating MapAnything process {proc.pid}")
        if proc.poll() is None:
            try:
                pgid = os.getpgid(proc.pid)
                print(f"DEBUG: Killing process group {pgid} with SIGTERM")
                os.killpg(pgid, signal.SIGTERM)
                proc.wait(timeout=10.0)
                print("DEBUG: Process group terminated successfully")
            except subprocess.TimeoutExpired:
                self.logger.warning(
                    "MapAnything service did not terminate in time; killing process group"
                )
                try:
                    pgid = os.getpgid(proc.pid)
                    print(f"DEBUG: Killing process group {pgid} with SIGKILL")
                    os.killpg(pgid, signal.SIGKILL)
                    proc.wait(timeout=5.0)
                    print("DEBUG: Process group killed with SIGKILL")
                except subprocess.TimeoutExpired:
                    self.logger.error(
                        "Unable to kill MapAnything service process group cleanly"
                    )
                except ProcessLookupError:
                    # Process group already dead
                    print("DEBUG: Process group already dead")
                    pass
            except ProcessLookupError:
                # Process already dead
                print("DEBUG: Process already dead")
                pass
        else:
            print(f"DEBUG: Process {proc.pid} already exited with code {proc.returncode}")
    ...
    def _start_websocket_server(self):
        ...
        def run_websocket_server():
            ...
            try:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Unable to set WebSocket server event loop: {e}"
                    )
                ...
                def shutdown_handler():
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Error stopping WebSocket server in thread: {e}"
                        )
                    finally:
                        # Stop the event loop
                        loop.stop()
                ...
                try:
                    ...
                    try:
                        ...
                        self.logger.debug(
                            f"WebSocket server already stopped or error during final cleanup: {e}"
                        )
                    finally:
        ...
        self.websocket_thread = threading.Thread(
            target=run_websocket_server, name="WebSocketThread", daemon=True
        )
        ...
        try:
            ...
            self.logger.info(
                f"Started WebSocket server thread on {self.config.websocket.HOST}:{self.config.websocket.PORT}"
            )
    ...
    def _start_mapanything_scheduler(self):
        ...
        if not self.multi_stream_processor:
            self.logger.warning(
                "MapAnything scheduler not started (no multi-stream processor)"
            )
            return
            ...
            while self.running and not self.stop_event.is_set():
                try:
                    ...
                    for source_id, info in source_info.items():
                        cam_id = str(
                            info.get("clean_name") or info.get("name") or source_id
                        )
                        try:
                            sensor_id_int = int(source_id)
                        except Exception:
                            sensor_id_int = None
                        ...
                        ok, jpeg_bytes, pts_us = (
                            self.multi_stream_processor.read_encoded_jpeg(
                                source_id, timeout=0.05
                            )
                        )
                        if not ok or not jpeg_bytes:
                            self.depth_source._pending_pts_us = None
                            continue
                        ...
                        # Run mono inference
                        try:
                            pending_pts = int(pts_us) if pts_us else None
                            self.depth_source._pending_pts_us = pending_pts
                            result = self.depth_source.maybe_infer_mono(
                                cam_id,
                                frame_bgr,
                            ...
                            if (
                                pending_pts is not None
                                and getattr(self.depth_source, "_pending_pts_us", None)
                                == pending_pts
                            ):
                                self.depth_source._pending_pts_us = None
                        except Exception as e:
                        ...
                        if (now - last_pub.get(cam_id, 0.0)) >= 5.0:
                            ...
                            if self._depth_publisher:
                                try:
                                    self._depth_publisher.publish_depth_summary(
                                        result, room_id
                                    )
                                except Exception as e:
                            ...
                            summary_message = {
                                ...
                                "summary": {
                                    ...
                                    "method": "mde"
                                    if result.summary.conf_mean
                                    >= getattr(self.depth_source, "min_conf", 0.5)
                                    else "floor",
                                },
                                "ts": result.ts_us,
                            }
    ...
    def _start_jpeg_processing_loop(self):
        ...
        def jpeg_processing_loop():
            ...
            if not source_info:
                self.logger.error(
                    "No source info available from multi-stream processor"
                )
                return
            ...
            self.logger.info(
                f"Processing JPEG data for {len(source_info)} sources: {list(source_info.keys())}"
            )
            ...
            for sid, info in source_info.items():
                ...
                if len(cam_id_bytes) <= 255:
                    self._ws_header_cache[camera_id] = (
                        bytes([len(cam_id_bytes)]) + cam_id_bytes
                    )
                else:
                    self.logger.error(
                        f"Camera ID too long to cache header: {camera_id}"
                    )
                ...
                try:
                    ...
                    if time.time() - heartbeat_t > 5:
                        try:
                            qsizes = {
                                sid: q.qsize()
                                for sid, q in self.multi_stream_processor.jpeg_queues.items()
                            }
                            self.logger.debug(
                                f"JPEG loop heartbeat – queue sizes: {qsizes}"
                            )
                        except Exception:
                            pass
                        heartbeat_t = time.time()
                    ...
                    for source_id, info in source_info.items():
                        ...
                        try:
                            q = getattr(
                                self.multi_stream_processor, "jpeg_queues", {}
                            ).get(source_id)
                        except Exception:
                            q = None
                        if q is not None:
                            try:
                                ...
                                while True:
                                    try:
                                        more = q.get_nowait()
                                        jpeg_bytes = (
                                            more[0] if isinstance(more, tuple) else more
                                        )
                                        drained += 1
                                    except queue.Empty:
                                        break
                        ...
                        if jpeg_bytes and self.websocket_server:
                            ...
                            if header is None:
                                ...
                                if len(cam_id_bytes) <= 255:
                                    ...
                                    self.logger.error(
                                        f"Camera ID too long: {len(cam_id_bytes)} bytes for {camera_id}"
                                    )
                                    continue
                            ...
                            self.rate_limited_logger.debug(
                                f"Broadcast JPEG frame for {camera_id}: {len(jpeg_bytes)} bytes"
                            )
                    ...
                    if time.time() - last_info_log >= 5.0:
                        try:
                            ...
                            qsizes = {}
                        summary_counts = {
                            source_id: frames_sent.get(source_id, 0)
                            for source_id in source_info.keys()
                        }
                        self.logger.debug(
                            f"📤 JPEG broadcast summary (last 5s): sent={summary_counts} | queues={qsizes} | clients={len(getattr(self.websocket_server, 'connected_clients', []))}"
                        )
        ...
        self.jpeg_thread = threading.Thread(
            target=jpeg_processing_loop, name="JPEGProcessingThread", daemon=True
        )
    ...
    def _start_result_processing(self):
        ...
        self.result_thread = threading.Thread(
            target=process_results, name="ResultProcessingThread", daemon=True
        )
    ...
    def _get_camera_ids(self) -> list:
        ids = []
        try:
            if self.multi_stream_processor and getattr(
                self.multi_stream_processor, "source_info", None
            ):
    ...
    def _load_calibration(self) -> None:
        ...
        self.calibration_bundle = assemble_calibration_bundle(
            cam_ids, self._intrinsics_models, model_map, extr, align, camera_specs
        )
        self._calib_paths = {
        ...
        camera_keys = sorted(
            list((self.calibration_bundle.get("cameras") or {}).get("K", {}).keys())
        )
    ...
    def _schedule_ws_broadcast(self, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            asyncio.run_coroutine_threadsafe(
                self.websocket_server.broadcast(message), loop
            )
    ...
    def _pixel_to_world_rpc(self, req: dict) -> dict:
        ...
        if intr is None or E is None:
            legacy_cam = (
                cameras_node.get(cam_id) if isinstance(cameras_node, dict) else None
            )
        ...
        if depth_result and depth_result.depth.size > 0:
            ...
            if 0 <= x_idx < w and 0 <= y_idx < h:
                ...
                if (
                    ...
                    and candidate_conf >= conf_threshold
                ):
        ...
        if depth_used is not None:
            try:
                world_point = pixel_to_world_aligned(
                    u, v, depth_used, cam_id, K, T_cam2world, align
                )
            except Exception as exc:
                try:
                    self.logger.debug(
                        "pixel_to_world depth transform failed for %s: %s", cam_id, exc
                    )
                except Exception:
                    pass
                world_point = None
    ...
    def _set_extrinsics_rpc(self, req: dict) -> dict:
        ...
        align = (
            self.calibration_bundle.get("align", {})
            if self.calibration_bundle
            else load_alignment(self._calib_paths.get("alignment", ""))
        )
        cam_ids = self._get_camera_ids()
        model_map = dict(self.config.calibration.CAMERA_INTRINSICS_MODEL_MAP)
        camera_specs = dict(getattr(self.config.calibration, "CAMERA_SPECS", {}) or {})
        self.calibration_bundle = assemble_calibration_bundle(
            cam_ids, self._intrinsics_models or {}, model_map, extr, align, camera_specs
        )
        if self.depth_source is not None:
            try:
                self.depth_source.calibration_bundle = self.calibration_bundle
    ...
    def _set_align_rpc(self, req: dict) -> dict:
        try:
            ...
            s_obj_to_m = (
                align_update.get("units", {}).get("s_obj_to_m")
                if isinstance(align_update.get("units"), dict)
                else None
            )
            ...
            model_map = dict(self.config.calibration.CAMERA_INTRINSICS_MODEL_MAP)
            camera_specs = dict(
                getattr(self.config.calibration, "CAMERA_SPECS", {}) or {}
            )
            self.calibration_bundle = assemble_calibration_bundle(
                cam_ids,
            ...
            self.logger.info(
                f"Updated alignment: matrix={bool(matrix)}, floor_y={floor_y}, s_obj_to_m={s_obj_to_m}"
            )
            return {"ok": True}
        except Exception as e:
    ...
    def _schedule_depth_inference(self, analysis_frame: AnalysisFrame) -> None:
        ...
        if (
            last_summary
            and last_summary.conf_mean is not None
            and last_summary.conf_mean >= 0.9
        ):
            try:
                if not analysis_frame.tracks:
                    self.logger.debug(
                        "Skipping MapAnything mono inference for %s (confidence %.2f, no motion)",
                        camera_id,
                        last_summary.conf_mean,
                    )
                    return
            except Exception:
                pass
        ...
        future.add_done_callback(
            lambda fut, cam=camera_id: self._handle_depth_future(cam, fut)
        )
        ...
        try:
            ...
            if result is not None:
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Depth cache update failed for {camera_id}: {exc}"
                    )
    ...
    def _process_analysis_frame(self, analysis_frame: AnalysisFrame):
        ...
        try:
            ...
            masks_count = sum(
                1 for det in analysis_frame.detections if det.mask is not None
            )
            self.rate_limited_logger.debug(
                f"Processing frame for camera {analysis_frame.camera_id}, frame_id={analysis_frame.frame_id}"
            )
            self.rate_limited_logger.debug(
                f"Frame has {len(analysis_frame.detections)} detections, {len(analysis_frame.tracks)} tracks, {masks_count} masks"
            )

            # Log detection count if > 0 (rate-limited)
            if len(analysis_frame.detections) > 0:
                self.rate_limited_logger.log_detection_count(
                    analysis_frame.camera_id,
                    len(analysis_frame.detections),
            ...
            if getattr(self.config.visualization, "USE_NATIVE_DEEPSTREAM_OSD", False):
                self.logger.info(
                    "Using native DeepStream OSD (nvdsosd) for visualization."
                )
                # Ensure frame is available for visualization
                if analysis_frame.frame is not None:
                    annotated_frame = (
                        analysis_frame.frame
                    )  # Already annotated by DeepStream
                else:
                    self.logger.warning(
                        f"No frame available for visualization, creating fallback frame"
                    )
                    annotated_frame = np.zeros(
                        (analysis_frame.frame_height, analysis_frame.frame_width, 3),
                        dtype=np.uint8,
                    )
            ...
            if not jpeg_data:
                self.logger.warning(
                    "JPEG encoding returned no data; skipping frame broadcast"
                )
                return
            ...
            self.logger.debug(
                f"Camera ID length: {camera_id_length}, JPEG length: {len(jpeg_data)}"
            )
            ...
            if self.config.output.SAVE_FRAMES:
                ...
                if self.config.output.FRAME_SAVE_INTERVAL > 1:
                    should_save = (
                        analysis_frame.frame_id - 1
                    ) % self.config.output.FRAME_SAVE_INTERVAL == 0

                if should_save:
                    output_path = (
                        Path(self.config.output.OUTPUT_DIR) / analysis_frame.camera_id
                    )
                    output_path.mkdir(parents=True, exist_ok=True)

                    frame_filename = (
                        output_path / f"frame_{analysis_frame.frame_id:06d}.jpg"
                    )
                    cv2.imwrite(str(frame_filename), annotated_frame)

                    # Log frame saving (rate-limited to avoid spam)
                    self.rate_limited_logger.debug(
                        f"Saved frame {analysis_frame.frame_id} for camera {analysis_frame.camera_id}"
                    )
    ...
    def _get_stats(self) -> Dict[str, Any]:
        ...
        def _sanitize_world(coords: Any) -> Optional[List[float]]:
            if isinstance(coords, (list, tuple, Sequence)) and len(coords) >= 3:
                values = [_safe_float(coords[0]), _safe_float(coords[1]), _safe_float(coords[2])]
                if all(v is not None for v in values):
                    return [float(v) for v in values]  # type: ignore[arg-type]
            return None

        def _sanitize_depth_meta(meta: Any) -> Optional[Dict[str, Any]]:
            if not isinstance(meta, Mapping):
                return None
            depth_payload: Dict[str, Any] = {}
            method = meta.get("method")
            if method is not None:
                depth_payload["method"] = str(method)

            for key in ("depth_m", "conf", "median", "p10", "p90", "valid_ratio", "conf_mean", "scale"):
                val = _safe_float(meta.get(key))
                if val is not None:
                    depth_payload[key] = val

            pts_us = _safe_int(meta.get("pts_us"))
            if pts_us is not None:
                depth_payload["pts_us"] = pts_us

            samples = _safe_int(meta.get("samples"))
            if samples is not None:
                depth_payload["samples"] = samples

            summary_count = _safe_int(meta.get("summary_sample_count"))
            if summary_count is not None:
                depth_payload["summary_sample_count"] = summary_count

            world_vals = _sanitize_world(meta.get("world"))
            if world_vals:
                depth_payload["world"] = world_vals

            pose = meta.get("pose")
            if isinstance(pose, (list, tuple, Sequence)):
                pose_vals = [_safe_float(v) for v in pose]
                if all(v is not None for v in pose_vals):
                    depth_payload["pose"] = [float(v) for v in pose_vals]  # type: ignore[arg-type]

            summary_meta = meta.get("summary")
            if isinstance(summary_meta, Mapping):
                summary_payload: Dict[str, Any] = {}
                for key in ("median", "p10", "p90", "conf_mean", "valid_ratio"):
                    val = _safe_float(summary_meta.get(key))
                    if val is not None:
                        summary_payload[key] = val
                sample_count = _safe_int(summary_meta.get("sample_count"))
                if sample_count is not None:
                    summary_payload["sample_count"] = sample_count
                if summary_payload:
                    depth_payload["summary"] = summary_payload
        ...
        try:
            ...
            def _normalize_room_name(name: Any) -> str:
                try:
                    ...
                    if (
                        " " not in s2
                        and any(c.islower() for c in s2)
                        and any(c.isupper() for c in s2)
                    ):
                        s2 = re.sub(r"(?<!^)(?=[A-Z])", " ", s2)
            ...
            stats = {
                ...
                "application": {
                    ...
                    "cameras_active": len(
                        [
                            s
                            for s in self.config.cameras.RTSP_STREAMS
                            if s.get("enabled", True)
                        ]
                    ),
                    "processors_active": 1
                    if hasattr(self, "multi_stream_processor")
                    and self.multi_stream_processor
                    else 0,
                },
                "cameras": {},
            }
            ...
            if hasattr(self, "multi_stream_processor") and self.multi_stream_processor:
                try:
                    ...
                    self.logger.debug(
                        f"📊 Multi-stream processor stats: {comprehensive_stats}"
                    )

                    depth_debug_entries: List[Dict[str, Any]] = []
                    if comprehensive_stats and "tracking" in comprehensive_stats:
                        ...
                        source_info = getattr(
                            self.multi_stream_processor, "source_info", {}
                        )

                        for (
                            source_id,
                            stream_tracking_data,
                        ) in per_stream_tracking.items():
                            if source_id in source_info:
                                camera_name = source_info[source_id][
                                    "clean_name"
                                ]  # e.g., 'living-room', 'kitchen', 'family-room'

                                # Create camera stats for this stream with normalized occupancy keys for frontend
                                try:
                                    raw_occ = (stream_tracking_data or {}).get(
                                        "occupancy", {}
                                    ) or {}
                                    norm_occ = {}
                                    ...
                                    raw_tracks = (
                                        stream_tracking_data or {}
                                    ).get("active_tracks", []) or []
                                    active_tracks_payload: List[Dict[str, Any]] = []
                                    for raw_track in raw_tracks:
                                        if not isinstance(raw_track, Mapping):
                                            continue
                                        track_wire: Dict[str, Any] = {}
                                        tid = _safe_int(raw_track.get("track_id"))
                                        if tid is not None:
                                            track_wire["track_id"] = tid
                                        stable_id = _safe_int(raw_track.get("stable_id"))
                                        if stable_id is not None:
                                            track_wire["stable_id"] = stable_id
                                        cam_id_val = raw_track.get("camera_id")
                                        if isinstance(cam_id_val, str):
                                            track_wire["camera_id"] = cam_id_val
                                        zone_val = raw_track.get("zone")
                                        if zone_val is not None:
                                            track_wire["zone"] = str(zone_val)
                                        dwell = _safe_float(raw_track.get("dwell_time"))
                                        if dwell is not None:
                                            track_wire["dwell_time"] = dwell
                                        bbox = raw_track.get("bbox")
                                        if isinstance(bbox, (list, tuple, Sequence)) and len(bbox) == 4:
                                            bbox_vals = [_safe_float(v) for v in bbox]
                                            if all(v is not None for v in bbox_vals):
                                                track_wire["bbox"] = [float(v) for v in bbox_vals]  # type: ignore[arg-type]
                                        center = raw_track.get("center")
                                        if isinstance(center, (list, tuple, Sequence)) and len(center) >= 2:
                                            cx = _safe_float(center[0])
                                            cy = _safe_float(center[1])
                                            if cx is not None and cy is not None:
                                                track_wire["center"] = [cx, cy]
                                        velocity = raw_track.get("velocity")
                                        if isinstance(velocity, (list, tuple, Sequence)) and len(velocity) >= 2:
                                            vx = _safe_float(velocity[0])
                                            vy = _safe_float(velocity[1])
                                            if vx is not None and vy is not None:
                                                track_wire["velocity"] = [vx, vy]
                                        tracker_conf = _safe_float(
                                            raw_track.get("tracker_confidence")
                                        )
                                        if tracker_conf is not None:
                                            track_wire["tracker_confidence"] = tracker_conf
                                        class_id_val = _safe_int(raw_track.get("class_id"))
                                        if class_id_val is not None:
                                            track_wire["class_id"] = class_id_val

                                        world_vals = _sanitize_world(raw_track.get("world"))
                                        depth_payload = _sanitize_depth_meta(
                                            raw_track.get("depth")
                                        )
                                        if world_vals:
                                            track_wire["world"] = world_vals
                                            track_wire["world_valid"] = bool(
                                                raw_track.get("world_valid", True)
                                            )
                                            if depth_payload and "world" not in depth_payload:
                                                depth_payload["world"] = list(world_vals)
                                        elif depth_payload and "world" in depth_payload:
                                            sanitized_world = _sanitize_world(
                                                depth_payload.get("world")
                                            )
                                            if sanitized_world:
                                                track_wire["world"] = sanitized_world
                                                track_wire["world_valid"] = True
                                                depth_payload["world"] = list(sanitized_world)

                                        if depth_payload:
                                            track_wire["depth"] = depth_payload
                                            try:
                                                depth_debug_entry = {
                                                    "camera": camera_name,
                                                    "track_id": track_wire.get("track_id"),
                                                    "stable_id": track_wire.get("stable_id"),
                                                    "depth": depth_payload.get("depth_m"),
                                                    "conf": depth_payload.get("conf"),
                                                    "method": depth_payload.get("method"),
                                                    "samples": depth_payload.get("samples"),
                                                    "world": depth_payload.get("world")
                                                    or track_wire.get("world"),
                                                }
                                                depth_debug_entries.append(depth_debug_entry)
                                            except Exception:
                                                pass
                                        else:
                                            raw_depth = raw_track.get("depth")
                                            if isinstance(raw_depth, Mapping):
                                                try:
                                                    converted_depth = convert_numpy_types(
                                                        {"depth": raw_depth}
                                                    ).get("depth")
                                                except Exception:
                                                    converted_depth = dict(raw_depth)
                                                if isinstance(converted_depth, Mapping):
                                                    depth_payload = dict(converted_depth)
                                                    track_wire["depth"] = depth_payload
                                                    try:
                                                        depth_debug_entries.append(
                                                            {
                                                                "camera": camera_name,
                                                                "track_id": track_wire.get(
                                                                    "track_id"
                                                                ),
                                                                "stable_id": track_wire.get(
                                                                    "stable_id"
                                                                ),
                                                                "depth": depth_payload.get(
                                                                    "depth_m"
                                                                ),
                                                                "conf": depth_payload.get(
                                                                    "conf"
                                                                ),
                                                                "method": depth_payload.get(
                                                                    "method"
                                                                ),
                                                                "samples": depth_payload.get(
                                                                    "samples"
                                                                ),
                                                                "world": depth_payload.get(
                                                                    "world"
                                                                )
                                                                or track_wire.get(
                                                                    "world"
                                        ...
                                        active_tracks_payload.append(track_wire)

                                    tracking_payload = {
                                        "active_tracks": active_tracks_payload,
                                        "transitions": (stream_tracking_data or {}).get(
                                            "transitions", []
                                        ),
                                        "occupancy": norm_occ,
                                    }
                                    ...
                                    try:
                                        ...
                                        k_table = (
                                            cameras_node.get("K")
                                            if isinstance(cameras_node, dict)
                                            else None
                                        )
                                        e_table = (
                                            cameras_node.get("E")
                                        ...
                                        intr = (
                                            (k_table or {}).get(camera_name)
                                            if isinstance(k_table, dict)
                                            else None
                                        )
                                        E = (
                                            (e_table or {}).get(camera_name)
                                            if isinstance(e_table, dict)
                                            else None
                                        )
                                        if (intr is None or E is None) and isinstance(
                                            cameras_node, dict
                                        ):
                                                ...
                                                for t in tracking_payload.get(
                                                    ...
                                                    try:
                                                        ...
                                                        if (
                                                            ...
                                                            depth_block = (
                                                                t.get("depth")
                                                                if isinstance(t.get("depth"), dict)
                                                                else None
                                                            )
                                                            if hit is not None:
                                                                world_vals = [
                                                                ...
                                                                if depth_block is not None:
                                                                    depth_block.setdefault(
                                                                        "world", world_vals
                                                                    )
                                                                    depth_val = _safe_float(
                                                                        depth_block.get("depth_m")
                                                                    )
                                                                    if depth_val is None or depth_val <= 0.0:
                                                                        try:
                                                                            depth_block["depth_m"] = math.sqrt(
                                                                                world_vals[0] ** 2
                                                                                + world_vals[1] ** 2
                                                                                + world_vals[2] ** 2
                                                                            )
                                                                ...
                                                                t["world_valid"] = False
                                                                # If depth exists but no world, flag it
                                                                if depth_block is not None:
                                                                    depth_block.setdefault("world", [])
                                                    except Exception:
                                                        try:
                                                            t["world_valid"] = False
                                                        except Exception:
                                ...
                                camera_stats = {
                                    "fps": comprehensive_stats.get("fps", 0)
                                    / len(per_stream_tracking)
                                    if per_stream_tracking
                                    else 0,
                                    "frames_processed": comprehensive_stats.get(
                                        "frames_processed", 0
                                    ),
                                    "status": "running"
                                    if comprehensive_stats.get("running", False)
                                    else "stopped",
                                ...
                                self.logger.debug(
                                    f"📊 Camera {camera_name} (source_id={source_id}) tracking data: {stream_tracking_data}"
                                )
                            else:
                                self.logger.warning(
                                    f"⚠️ Unknown source_id {source_id} in tracking data"
                                )

                        if depth_debug_entries:
                            try:
                                self.logger.info(
                                    "WebSocket stats depth payloads: %s",
                                    depth_debug_entries,
                                )
                            except Exception:
                                pass
                    ...
                    self.logger.error(
                        f"Error getting multi-stream processor stats: {e}"
                    )
    ...
    def _get_detection_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "detection_toggles": {
                ...
                "detect_vehicles": any(
                    c in getattr(self.config.models, "TARGET_CLASSES", [])
                    for c in [1, 2, 3, 5, 7, 8]
                ),
                "detect_furniture": any(
                    c in getattr(self.config.models, "TARGET_CLASSES", [])
                    for c in [13, 56, 57, 59, 60, 61]
                ),
            },
        }
    ...
    def _handle_depth_panel_state(self, is_open: bool) -> None:
        """Toggle MapAnything inference based on depth drawer visibility."""
        state = bool(is_open)
        if getattr(self, "_depth_panel_open", False) == state:
            return
        self._depth_panel_open = state
        self.logger.info(f"Depth panel {'opened' if state else 'closed'} via frontend")
        if not self.depth_source:
            return
        if state:
            self.depth_source.enable_inference(True, source="frontend_depth_panel")
            return
        auto_disable = True
        if hasattr(self.depth_source, "auto_disable_enabled"):
            try:
                auto_disable = bool(self.depth_source.auto_disable_enabled())
            except Exception:
                auto_disable = True
        if auto_disable:
            self.depth_source.enable_inference(False, source="frontend_depth_panel")
        else:
            self.logger.debug(
                "Depth panel closed but auto-disable disabled; keeping inference active"
            )

    def _handle_intrinsics_ready(self) -> None:
        try:
            if self.multi_stream_processor:
                self.multi_stream_processor.open_mapanything_branch()
                self.logger.info(
                    "MapAnything Triton branch opened (intrinsics ready)"
                )
        except Exception as exc:
            self.logger.warning(
                f"Unable to open MapAnything Triton branch when intrinsics ready: {exc}"
            )
    ...
    def _handle_toggle_update(self, toggle_name: str, enabled: bool):
        ...
        if toggle_name == "trail_visualization_enabled":
            ...
            self.logger.info(
                f"Updated visualization config: {toggle_name.upper()} = {enabled}"
            )
        else:
        ...
        try:
            ...
            if "confidence_threshold" in config_data:
                ...
                if (
                    ...
                    self.multi_stream_processor.update_confidence_threshold(
                        new_threshold
                    )
                    self.logger.info(
                        "Updated confidence threshold for multi-stream processor"
                    )
            ...
            if "detection_enabled" in config_data:
                ...
                if (
                    ...
                    self.logger.info(
                        f"Updated detection enabled for multi-stream processor: {enabled}"
                    )
        ...
        try:
            ...
            class_mapping = {
                ...
                "detect_furniture": [
                    ...
                    60,
                    61,
                ],  # bench, chair, couch, dining table, tv, laptop
            }
            ...
            if toggle_name in class_mapping:
                ...
                if (
                    ...
                    if enabled:
                        ...
                        current_classes = getattr(
                            self.config.models, "TARGET_CLASSES", []
                        )
                        ...
                        self.logger.info(
                            f"Added classes {target_classes} to multi-stream processor"
                        )
                    else:
                        ...
                        new_classes = [
                            c for c in current_classes if c not in target_classes
                        ]
                        self.config.models.TARGET_CLASSES = new_classes
                        self.multi_stream_processor.update_target_classes(new_classes)
                        self.logger.info(
                            f"Removed classes {target_classes} from multi-stream processor"
                        )
            else:
    ...
    def stop(self):
        ...
        if torch.cuda.is_available() and torch.cuda.device_count() > 0:
            ...
            try:
                ...
                if (
                    ...
                    try:
                        ...
                        self.logger.info(
                            "Early cleanup: Multi-stream processor GPU resources"
                        )
                    except Exception as e:
                        self.logger.warning(
                            f"Error in early GPU cleanup for multi-stream processor: {e}"
                        )

                # Force cleanup all GPU resources via consolidated pipeline
                # cleanup_all_gpu_resources() # This function is deprecated
                self.logger.info(
                    "Early cleanup: all GPU resources via unified pipeline"
                )
            ...
            self.logger.warning(
                "CUDA not available or no devices, skipping GPU cleanup"
            )
            ...
            try:
                ...
                if hasattr(self, "websocket_loop") and self.websocket_loop:
                    ...
                    try:
                        ...
                        self.logger.debug(
                            f"Could not mark WebSocket server as not running: {exc}"
                        )
                ...
                if stop_ok:
                    ...
                    self.logger.warning(
                        "⚠️ WebSocket server stop did not confirm completion"
                    )
            except Exception as e:
        ...
        if hasattr(self, "websocket_thread") and self.websocket_thread:
            try:
                ...
                thread_joined = safe_join(
                    self.websocket_thread, timeout=5.0, name="websocket_thread"
                )
                if thread_joined:
                    self.logger.info("✅ WebSocket thread stopped gracefully")
                else:
                    self.logger.warning(
                        "⚠️ WebSocket thread did not stop within timeout, will be abandoned"
                    )
            except Exception as e:
    ...
    def _cleanup_pytorch_jit_temp_dirs(self):
        ...
        try:
            cleaned_count = 0
            for tmp_dir in glob.glob('/tmp/tmp*'):
                try:
                    # Check if it's a PyTorch JIT cache dir (contains _remote_module files)
                    if os.path.isdir(tmp_dir):
                        contents = os.listdir(tmp_dir)
                        if any('_remote_module' in f for f in contents):
                            shutil.rmtree(tmp_dir, ignore_errors=True)
                            cleaned_count += 1
                except Exception as e:
                    self.logger.debug(f"Could not clean temp dir {tmp_dir}: {e}")
            if cleaned_count > 0:
                self.logger.info(f"Cleaned up {cleaned_count} PyTorch JIT temp directories")
        except Exception as e:
            self.logger.debug(f"Error during PyTorch JIT temp cleanup: {e}")
...
def parse_arguments():
    ...
    # Processing options
    parser.add_argument(
        "--gpu-device", type=str, default="cuda:0", help="GPU device to use"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--target-classes", type=int, nargs="+", help="Target class IDs to detect"
    )
    parser.add_argument(
        "--analysis-frame-interval",
        type=int,
        help="Process every Nth frame for analysis (default: 15)",
    )
    ...
    parser.add_argument(
        "--save-detections", action="store_true", help="Save detection results"
    )
    parser.add_argument(
        "--frame-save-interval", type=int, help="Save every Nth frame (default: 15)"
    )
    ...
    parser.add_argument(
        "--enable-nvdec", action="store_true", help="Enable NVDEC hardware decoding"
    )
    parser.add_argument(
        "--enable-gpu-preprocessing",
        action="store_true",
        help="Enable GPU preprocessing",
    )
    parser.add_argument(
        "--force-gpu-only",
        action="store_true",
        help="Force GPU-only processing (no CPU fallbacks)",
    )

    # WebSocket options
    parser.add_argument(
        "--websocket-host", type=str, default="0.0.0.0", help="WebSocket server host"
    )
    parser.add_argument(
        "--websocket-port", type=int, default=6008, help="WebSocket server port"
    )

    # Visualization options
    parser.add_argument(
        "--show-traces", action="store_true", help="Show tracking traces"
    )
    parser.add_argument(
        "--show-detection-boxes", action="store_true", help="Show detection boxes"
    )
    parser.add_argument(
        "--show-tracking-boxes", action="store_true", help="Show tracking boxes"
    )
    parser.add_argument(
        "--show-keypoints", action="store_true", help="Show pose keypoints"
    )
    parser.add_argument(
        "--show-masks", action="store_true", help="Show segmentation masks"
    )
...
def main():
    ...
    # Set up logging
    setup_logging(log_level=config.app.LOG_LEVEL, log_file=config.app.LOG_FILE)
