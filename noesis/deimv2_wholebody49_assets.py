...
def resolve_wholebody49_assets(size: str) -> Dict[str, Path | str | int | bool]:
    ...
    return {
        ...
        "template": (REPO_ROOT / "pipelines" / variant.template_name).resolve(),
        "preproc_template": (REPO_ROOT / "pipelines" / "config_preproc.ini").resolve(),
        "onnx": (REPO_ROOT / "models" / "onnx" / variant.onnx_name).resolve(),
        "engine": (REPO_ROOT / "models" / "engines" / variant.engine_name).resolve(),
        "labels": (REPO_ROOT / "models" / "deimv2_wholebody49" / "classes.txt").resolve(),
        "parser": (
            REPO_ROOT
            / "pipelines"
        ...
        "default_output": (REPO_ROOT / "build" / f"config_infer_primary_wholebody49_{size_norm}.ini").resolve(),
    }
...
def materialize_wholebody49_pgie_ini(
    *,
    ...
    replacements = {
        "@ONNX_PATH@": str(Path(assets["onnx"]).resolve()),
...
def materialize_wholebody49_configs(
    *,
    ...
    preprocess_config = materialize_wholebody49_preproc_config(
        output_path=REPO_ROOT / "build" / f"config_preproc_wholebody49_{size_norm}_b{int(batch_size)}.ini",
        batch_size=batch_size,
        src_ids=src_ids,
        logger=logger,
    )
