...
from noesis.pipelines import ds8_pipeline, hooks_v3dt_reimpl as hooks
...
def _build_dir() -> Path:
    return Path(
        os.environ.get("NOESIS_BUILD_DIR", REPO_ROOT / "build")
    ).expanduser().resolve()


def _prepend_env_paths(name: str, paths: list[Path]) -> None:
    current = [item for item in str(os.environ.get(name, "") or "").split(os.pathsep) if item]
    incoming = [str(path) for path in paths if path.exists()]
    merged: list[str] = []
...
def _configure_v3dt_patched_nvtracker_plugin(logger: logging.Logger) -> bool:
    """Load the repo-local nvtracker patch only for the isolated V3DT runtime."""
    flag = str(os.environ.get("NOESIS_V3DT_USE_PATCHED_NVTRACKER", "1") or "").strip().lower()
    if flag in _ENV_FALSE:
        logger.error(
            "V3DT patched nvtracker plugin disabled; refusing to run because the stock "
            "tracker draws an internal 3D cuboid that breaks visual bbox validation."
        )
        return False

    override_deepstream = (REPO_ROOT / "build" / "gst-plugins-deepstream").resolve()
    override_system = (REPO_ROOT / "build" / "gst-plugins-system").resolve()
    patched_plugin = override_deepstream / "libnvdsgst_tracker.so"
    if not patched_plugin.exists():
        logger.error(
            "V3DT patched nvtracker plugin missing: %s. Run scripts/build_patched_nvtracker.sh "
            "before starting the V3DT reimplementation runtime.",
            patched_plugin,
        )
        return False

    plugin_paths = [override_deepstream]
    if override_system.exists():
        plugin_paths.append(override_system)
    _prepend_env_paths("GST_PLUGIN_PATH", plugin_paths)
    _prepend_env_paths("GST_PLUGIN_SYSTEM_PATH", plugin_paths)

    registry_path = Path(
        os.environ.get("GST_REGISTRY_1_0", _build_dir() / "gst-registry-v3dt.bin")
    ).expanduser().resolve()
    registry_path.parent.mkdir(parents=True, exist_ok=True)
    os.environ["GST_REGISTRY_1_0"] = str(registry_path)

    logger.info("V3DT patched nvtracker enabled: %s", patched_plugin)
    logger.info("V3DT GST_PLUGIN_PATH=%s", os.environ.get("GST_PLUGIN_PATH", ""))
    return True
...
def _build_stable_id_manager(
    ...
    *,
    ...
    try:
        stable_id_cfg: Mapping[str, Any] = {}
        try:
            cfg_candidate = (pipeline_config or {}).get("stable_id") or {}
            if isinstance(cfg_candidate, Mapping):
                stable_id_cfg = cfg_candidate
        except Exception:
            stable_id_cfg = {}

        def _cfg_value(*keys: str, default: Any = None) -> Any:
            for key in keys:
                if key in stable_id_cfg:
                    value = stable_id_cfg.get(key)
                    if value is not None:
                        return value
            return default

        def _env_or_cfg(env_name: str, *keys: str, default: Any = None) -> Any:
            raw = os.environ.get(env_name)
            if raw is not None and str(raw).strip() != "":
                return raw
            return _cfg_value(*keys, default=default)
        ...
        def _resolve_state_path(value: Any) -> str:
        ...
        embed_interval_s = _as_float(_env_or_cfg("NOESIS_REID_EMBED_INTERVAL_S", "embed_interval_s", default=0.5), 0.5)
        new_id_hysteresis_frames = _as_int(
            _env_or_cfg("NOESIS_REID_NEW_ID_HYSTERESIS_FRAMES", "new_id_hysteresis_frames", default=1),
            1,
        )
        new_id_confirm_frames_at_cap = _as_int(
            _env_or_cfg("NOESIS_REID_NEW_ID_CONFIRM_FRAMES_AT_CAP", "new_id_confirm_frames_at_cap", default=1),
            1,
        )
        ...
        active_evict_grace_s = _as_float(
            _env_or_cfg("NOESIS_REID_ACTIVE_EVICT_GRACE_S", "active_evict_grace_s", default=10.0),
            10.0,
        )
        xcam_handoff_window_s = _as_float(
            _env_or_cfg("NOESIS_REID_XCAM_HANDOFF_WINDOW_S", "xcam_handoff_window_s", default=4.0),
            4.0,
        )
        xcam_handoff_margin = _as_float(
            _env_or_cfg("NOESIS_REID_XCAM_HANDOFF_MARGIN", "xcam_handoff_margin", default=0.02),
            0.02,
        )
        allow_multi_zone_active = _as_bool(
            _env_or_cfg("NOESIS_REID_ALLOW_MULTI_ZONE_ACTIVE", "allow_multi_zone_active", default=True),
            True,
        )
        aliases_enabled = _as_bool(
            _env_or_cfg("NOESIS_REID_ALIASES_ENABLED", "aliases_enabled", default=True),
            True,
        )
        ...
        if reset_sid_pool_env is None:
            ...
            reset_sid_pool_env = _env_or_cfg("NOESIS_REID_RESER_SID_POOL", "reset_sid_pool_on_start", "reset_sid_pool", default=False)
        reset_sid_pool = _as_bool(reset_sid_pool_env, False)
        max_total_ids = _as_int(_env_or_cfg("NOESIS_REID_MAX_TOTAL_IDS", "max_total_ids", default=12), 12)
        total_id_reuse = _as_bool(_env_or_cfg("NOESIS_REID_TOTAL_ID_REUSE", "total_id_reuse", default=True), True)
        total_id_reuse_min_age_s = _as_float(
            _env_or_cfg("NOESIS_REID_TOTAL_ID_REUSE_MIN_AGE_S", "total_id_reuse_min_age_s", default=60.0),
            60.0,
        )
        auto_merge_enabled = _as_bool(
            _env_or_cfg("NOESIS_REID_AUTO_MERGE_ENABLED", "auto_merge_enabled", default=True),
            True,
        )
        auto_merge_interval_s = _as_float(
            _env_or_cfg("NOESIS_REID_AUTO_MERGE_INTERVAL_S", "auto_merge_interval_s", default=5.0),
            5.0,
        )
        auto_merge_max_attempts = _as_int(
            _env_or_cfg("NOESIS_REID_AUTO_MERGE_MAX_ATTEMPTS", "auto_merge_max_attempts", default=20),
            20,
        )
        auto_merge_min_sim = _as_float(
            _env_or_cfg("NOESIS_REID_AUTO_MERGE_MIN_SIM", "auto_merge_min_sim", default=0.92),
            0.92,
        )
        auto_merge_min_embeddings_for_suggest = _as_int(
            _env_or_cfg(
                "NOESIS_REID_AUTO_MERGE_MIN_EMBEDDINGS_FOR_SUGGEST",
                "auto_merge_min_embeddings_for_suggest",
                default=1,
            ),
            1,
        )
        auto_merge_respect_inactive = _as_bool(
            _env_or_cfg("NOESIS_REID_AUTO_MERGE_RESPECT_INACTIVE", "auto_merge_respect_inactive", default=False),
            False,
        )
        auto_merge_force_stuck = _as_bool(
            _env_or_cfg("NOESIS_REID_AUTO_MERGE_FORCE_STUCK", "auto_merge_force_stuck", default=False),
            False,
        )
        auto_merge_allow_both_active = _as_bool(
            _env_or_cfg("NOESIS_REID_AUTO_MERGE_ALLOW_BOTH_ACTIVE", "auto_merge_allow_both_active", default=True),
            True,
        )
        auto_merge_both_active_min_sim = _as_float(
            _env_or_cfg("NOESIS_REID_AUTO_MERGE_BOTH_ACTIVE_MIN_SIM", "auto_merge_both_active_min_sim", default=0.97),
            0.97,
        )
        alias_file = _resolve_state_path(_env_or_cfg("NOESIS_REID_ALIAS_FILE", "alias_file", default="~/.noesis/reid_aliases.json"))
        sid_pool_file = _resolve_state_path(_env_or_cfg("NOESIS_REID_SID_POOL_FILE", "sid_pool_file", default="~/.noesis/sid_pool.json"))
        alias_append_default = _as_bool(
            _env_or_cfg("NOESIS_REID_ALIAS_APPEND_DEFAULT", "alias_append_default", default=True),
            True,
        )
        try:
        ...
        try:
            alias_history_max = _as_int(_env_or_cfg("NOESIS_REID_ALIAS_HISTORY_MAX", "alias_history_max", default=1000), 1000)
        except Exception:
        ...
        extra_kwargs = {
            ...
            "cos_sim_high_threshold": cos_sim_high_threshold,
            "max_active_ids_per_sensor": max_active_ids_per_sensor,
            "active_evict_grace_s": active_evict_grace_s,
...
def _build_rest_app() -> "FastAPI":
    ...
    from noesis.server import analytics_api, depth_api, health_api, reid_api, reid_v2_api, scene_api, virtual_twin_api
...
def _run_main(startup_main_guard: StartupMainGuard) -> int:
    ...
    if tracking_mode == "v3dt":
        ...
        if not _validate_v3dt_tracking_guardrails(pipeline_path, logger):
            return 1
        if not _configure_v3dt_patched_nvtracker_plugin(logger):
            return 1
    else:
    ...
    def _ds8_floorplan_provider(
        ...
        **_ignored: object,
        ...
        return providers.floorplan_provider(
            camera,
    ...
    if tracking_mode == "baseline":
        try:
            ...
            return _abort_startup("depth_registration_invalid")
    ...
    if tracking_mode == "v3dt":
        try:
            hooks.attach_person_bbox_overlay_hook(pipeline)
        except Exception:
            logger.exception("Failed to attach V3DT person bbox overlay hook")
    try:
    ...
    def _tracking_contract_metadata(source_id: int, tracks: list[Mapping[str, Any]]) -> Dict[str, Any]:
        ...
        units = "meters"
        image_size = None
        world_source = "v3dt_bbox3d_foot" if tracking_mode == "v3dt" else "backend_world_fused"
        try:
            bundle = calibration_provider.calibration_bundle()
    ...
    startup_transaction.bind(
        "health_api_bindings",
        lambda: health_api.register_capability_monitor_getter(
            ...
            try:
                ...
                try:
                    initial_webrtc_clients = max(
                        0,
                        int(os.environ.get("NOESIS_MOSAIC_WEBRTC_INITIAL_CLIENTS", "1")),
                    )
                except Exception:
