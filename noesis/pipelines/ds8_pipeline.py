...
def build_pipeline(yaml_path: str | Path) -> DS8Pipeline:
    ...
    def _nvinfer_props(raw: Dict[str, Any]) -> Dict[str, Any]:
        ...
        cfg.pop("layer", None)
        # Product embedding contract consumed by identity, not a GStreamer property.
    ...
    # Mosaic encode branch: raw surfaces → NVENC → H.264 AUs.
    # WebRTC consumes AUs through shmsink (no UDP loopback / RTSP hop).
    ...
    if mosaic_encode_enabled:
        ...
        if mosaic_webrtc_enabled:
            mosaic_branch["webrtc_queue"] = Component(
            ...
            mosaic_branch["shmsink"] = Component(
        ...
        if rtsp_enabled:
            ...
            mosaic_branch["rtsp_queue"] = Component(
            ...
            mosaic_branch["rtsp_out"] = Component(
                name="rtsp_out",
    ...
    # Orderly shutdown EOS must reach every terminal even while optional
    # branches are gated.  Valve mode 1/2 forwards sticky events (EOS is
    # sticky); the default mode 0 would acknowledge the upstream push while
    # silently preventing Service Maker's wait loop from terminating.
