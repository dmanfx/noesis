# Historical agent-policy snapshot — Menon world unification

**Non-operative archive:** this file preserves the local instructions that
applied while the completed workstream was active. It is not an `AGENTS.md`,
does not define current scope or precedence, and must not override repository
or `plans/` guidance.

The original snapshot follows unchanged apart from this status banner.
