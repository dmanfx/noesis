...
def main() -> int:
    ...
    if args.pipeline_config is None and args.tracking_mode is None:
        default_pipeline = (
            "config/infer_v3dt_sv3dt.yaml"
            if Path("config/infer_v3dt_sv3dt.yaml").exists()
            else "config/infer_v3dt.yaml"
        )
        if Path("config/infer_v3dt_sample.yaml").exists():
            default_pipeline = "config/infer_v3dt_sample.yaml"
        args.pipeline_config = default_pipeline
        if args.cameras_config is None:
            args.cameras_config = "config/cameras.yaml"
    ...
    try:
        ...
        if not ok:
            ...
            if "No tracking messages observed" in str(last_err):
                hint = (
                    " Ensure a person is visible to the camera(s), or run with "
                    "`--pipeline-config config/infer_v3dt_sample.yaml` (offline Retail02 clip)."
                )
            exit_note = ""
            if proc is not None and proc.poll() is not None:
