"""Entry point for the YOLO26 instance segmentation test pipeline."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DS8 YOLO26 segmentation test pipeline")
    parser.add_argument(
        "--camera",
        type=str,
        default=None,
...
def _build_nvinfer_config(msize: str, parser_lib: Path) -> Path:
    root = _repo_root()
    onnx_path = root / f"models/yolo26{msize}-seg.onnx"
    engine_path = root / f"models/engines/yolo26{msize}-seg_b3_fp16.engine"
    labels_path = Path(__file__).resolve().parent / "labels.txt"
    template_path = Path(__file__).resolve().parent / "config_infer_yolo26_seg.template.ini"
    ...
    output_path = output_dir / f"config_infer_yolo26_seg_{msize}.ini"
    ...
    text = text.replace("@CUSTOM_LIB@", str(parser_lib.resolve()))
...
def main() -> None:
    ...
    parser_lib = parser_setup.ensure_custom_parser_lib()
    nvinfer_config = _build_nvinfer_config(args.msize, parser_lib)
