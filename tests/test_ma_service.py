...
def test_infer_mono_returns_flat_depth_map():
    ...
    depth = np.frombuffer(base64.b64decode(payload['depth_b64']), dtype=np.float32).reshape(shape)
...
def test_infer_multi_returns_consistent_flat_depth_maps():
    ...
    for cam_id in ('cam-a', 'cam-b'):
        shape = tuple(int(dim) for dim in payload['shapes'][cam_id])
        depth = np.frombuffer(base64.b64decode(payload['depth_b64'][cam_id]), dtype=np.float32).reshape(shape)
        assert depth.shape == (256, 256)
        assert np.allclose(depth, 2.0, atol=0.01)


@pytest.mark.parametrize("headers", [{}, {"X-API-Key": "invalid"}])
def test_inference_rejects_missing_or_invalid_api_key(headers):
    client = TestClient(server.app)
    rgb = _build_flat_depth_rgb(height=8, width=8)
    response = client.post(
        "/infer_mono",
        json={"view": encode_image(rgb, "mono-cam")},
        headers=headers,
