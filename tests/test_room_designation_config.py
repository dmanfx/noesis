...
def test_overcrowding_is_the_single_ds9_room_designation() -> None:
    post = _post_stage(REPO_ROOT / "DS9" / "config" / "nvdsanalytics.yaml")
    ini = _post_ini(
        REPO_ROOT / "DS9" / "config" / "config_nvdsanalytics_post.ini"
    )

    assert set(post["streams"]) == set(EXPECTED_ROOM_IDS)
    for stream_id, room_id in EXPECTED_ROOM_IDS.items():
        stream = post["streams"][stream_id]
